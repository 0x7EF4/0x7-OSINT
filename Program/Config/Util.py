...
def Censored(text):
    ...
    for censored_text in censored:
        if text in censored:
            print(f'{BEFORE + current_time_hour() + AFTER} {ERROR} Unable to find "{white}{text}{purple}".')
            Continue()
            Reset()
...
def Reset():
    if os_name == "Windows":
        file = ['python', os.path.join(tool_path, "0x7EF4.py")]
        subprocess.run(file)

    elif os_name == "Linux":
        file = ['python3', os.path.join(tool_path, "0x7EF4.py")]
        subprocess.run(file)
